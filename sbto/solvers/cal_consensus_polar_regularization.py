...
def compute_polar_consensus(
    ...
    #   ...
    # ]
    # for vanilla CBO, there is only one consensus point,
    # to calculate this consensus point, there is N cost
    # for polarized CBO, each particle has its own consensus point and
    # there is N regularized cost for each particlei,
    # row i: J_i+r_{i,j} with j dummy
    ...
    # Sanity check: when polar_kernel_reg_loss_weight == 0, each column should match.
    def _check_columns(_):
        ...
        def _bad(_):
            ...
            jax.debug.callback(_raise, None)
            return None
    ...
    # the weights for consensus of particle j. Broadcast x along a new column
    # axis so weights[i, j] multiplies x[i].
...
def persist_consensus_stats(
    consensus: np.ndarray, stats_dir: Path, iteration: int
) -> None:
    stats_dir.mkdir(parents=True, exist_ok=True)
    out_path = stats_dir / "polar_consensus_stats.csv"
    ...
    if consensus_np.ndim < 2:
        return
    num_particles = consensus_np.shape[0]

    per_particle_vars = [float(np.var(consensus_np[idx].reshape(-1))) for idx in range(num_particles)]
    var_all = float(np.mean(per_particle_vars))
    knot_vars = []
    for knot_idx in range(consensus_np.shape[1]):
        knot_slice = consensus_np[:, knot_idx].reshape(num_particles, -1)
        knot_vars.append(float(np.var(knot_slice)))

    print(
        f"[polar_consensus_stats] iter={iteration_val} "
        f"particles={num_particles} "
        f"var_all_mean={var_all:.6f}"
    )

    with out_path.open("a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if write_header:
            header = ["iteration", "variance_all"]
            header += [f"variance_knot_{i}" for i in range(consensus_np.shape[1])]
            writer.writerow(header)
        writer.writerow([iteration_val, var_all, *knot_vars])
